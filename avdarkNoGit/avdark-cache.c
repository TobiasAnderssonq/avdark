...
avdc_access(avdark_cache_t *self, avdc_pa_t pa, avdc_access_type_t type)
{
		...
		for (int i = 0; i < sets->assoc; i++) {
			hit = self->sets[index].lines[i].valid && self->sets[index].lines[i].tag == tag;

			if(hit) {
				self->sets[index].lines[i].age = 0;
				found = 1;
			} else{
				self->sets[index].lines[i].age = self->sets[index].lines[i].age++;	
			}
		}
